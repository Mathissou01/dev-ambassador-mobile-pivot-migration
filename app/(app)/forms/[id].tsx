import React, {ReactNode, type SetStateAction, useContext, useEffect, useState} from "react";
...
export default function FormScreen(): ReactNode {
    ...
    const sendForm = (field: any): void => {
        ...
        const manageResponse = (resp: { inlineMessage?: string; redirectUri?: string }): void => {
            if ((resp?.inlineMessage ?? resp?.redirectUri) != null) {
                ...
                setPopupMessage("Erreur d'enregistrement ! Envoyer de nouveau dans quelques minutes.");
                setPopupCallback(() => () => {
                }); // Logique de redirection ou de callback
                setShowPopup(true);
        ...
        if (idUser != null) {
            ...
            const askForMail = (firstAsk: boolean = false): void => {
                Alert.prompt(
                    ...
                    (firstAsk ? "" : "E-mail non valide. ") + "Veuillez renseigner l'e-mail de l'ambassador.",
                    [
                        {
                            text: "OK",
        ...
        switch (inputField.type) {
            case "select":
                selectsOpen[inputField.name] =
                    selectsOpen[inputField.name] === undefined ? false : selectsOpen[inputField.name];
                return (
                    <Controller
                        ...
                        render={({field}) => {
                            return (
                                <View style={{zIndex: selectsOpen[inputField.name] ? 1000 : 0}}>
                                    <LabelTemplate
                            ...
                            return (
                                <>
                                    ...
                                    {formState.errors[inputField.name] != null && (
                                        <ErrorMessageTemplate errors={formState?.errors?.[inputField.name]}/>
                                    )}
                                </>
                            );
                        }}
                            ...
                            return (
                                <>
                                    <LabelTemplate
                                        ...
                                        required={inputField.isRequired}
                                    />
                                    <DateTimePickerTemplate date={field.value} setDate={field.onChange}/>
                                </>
                            );
                        }}
                        name={inputField.name}
