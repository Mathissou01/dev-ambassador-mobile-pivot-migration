...
import { type resourceMappingEyebrows } from "@/components/Account/EditCharacterBlock/CharacterPart/ressourceMappingEyebrows";
import { type resourceMappingEarings } from "@/components/Account/EditCharacterBlock/CharacterPart/ressourceMappingEarings";
import { type resourceMappingGlass } from "@/components/Account/EditCharacterBlock/CharacterPart/ressourceMappingGlass";
import { type resourceMappingNecklace } from "@/components/Account/EditCharacterBlock/CharacterPart/ressourceMappingNecklace";
...
export default function CharacterEdit(): ReactNode {
    ...
    if (glViewRef.current) {
      ...
      try {
        ...
        if (status === "granted") {
          ...
          console.log(
            "Capture d'écran enregistrée avec succès dans la galerie."
          );
        } else {
        ...
        console.log(
          "Erreur lors de l'enregistrement de la capture d'écran :",
          error
        );
      }
    }
  ...
  const [selectedItem, setSelectedItem] = useState<
    keyof typeof resourceMappingHair
  >(userInfos?.avatar?.hair ?? "hair1");
  const [selectedItem2, setSelectedItem2] = useState<
    keyof typeof resourceMappingHead
  >(userInfos?.avatar?.head ?? "head1");
  const [selectedItem3, setSelectedItem3] = useState<
    keyof typeof resourceMappingEyebrows
  >(userInfos?.avatar?.eyebrows ?? "eyebrows1");
  const [selectedItem4, setSelectedItem4] = useState<
    keyof typeof resourceMappingEarings
  >(userInfos?.avatar?.earings ?? "earings1");
  const [selectedItem5, setSelectedItem5] = useState<
    keyof typeof resourceMappingGlass
  >(userInfos?.avatar?.glass ?? "glass1");
  const [selectedItem6, setSelectedItem6] = useState<
    keyof typeof resourceMappingNecklace
  >(userInfos?.avatar?.necklace ?? "necklace1");
  const [selectedItem7, setSelectedItem7] = useState<
    keyof typeof resourceMappingClothe
  >(userInfos?.avatar?.clothe ?? "clothe5");
  const [selectedItem12, setSelectedItem12] = useState<
    keyof typeof resourceMappingSkin
          ...
          >
            ...
            >
              <Background
                ...
                mainColor={
                  ColorToDisplay[
                    userInfos?.school?.name ?? "Bachelor Institute"
                  ]?.mainColor
                }
                secondColor={
                  ColorToDisplay[
            ...
            <View style={styles.confirmSliderContainer}>
              <TouchableOpacity
                ...
                style={[
                  ...
                  {
                    backgroundColor:
                      themeContext?.colors.primaryExtraLight + "4D",
                  },
                ]}
              >
                <CheckIcon
