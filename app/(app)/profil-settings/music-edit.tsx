...
const componentsWithMusic: MusicComponent[] = [
  ...
  },
  {
    ...
    subtitle: "Next-U",
    tag: "Next Remix",
    music: require("@/assets/sounds/AvatarMusic/SURPRISE.mp3"),
  },
];
...
export default function MusicEditScreen(): React.JSX.Element {
      ...
      if (typeof index !== "undefined") {
        ...
        setCurrentIndex((prevIndex) =>
          isNext
            ...
            : (prevIndex - 1 + componentsWithMusic.length) %
              componentsWithMusic.length
        );
      }
      setTimeout(() => {
        Animated.sequence([
          Animated.timing(viewYPosition, {
            toValue: -20,
            duration: 150,
            useNativeDriver: true,
          }),
  ...
  const rotateVinyle = (): void => {
    if (isPlaying) {
      ...
      const duration =
        startTime.current > 0 ? currentTime - startTime.current : 4000;
    ...
    navigation.setOptions({
      headerRight: () => (
        ...
        >
          <InfoIcon
            size={24}
            color={colorTheme?.isDark ?? false ? "white" : "black"}
          />
        </TouchableOpacity>
      ),
    });
      ...
      >
        ...
        <View style={styles.overlay}>
          <View style={styles.gestureVynilContainer}>
            <Animated.View
              style={{ transform: [{ translateY: viewYPosition }] }}
            >
              <Animated.View
                style={{ transform: [{ rotate: rotateInterpolated }] }}
              >
                <Vinyle
                  centerImage={{
                    uri: avatarImage!,
