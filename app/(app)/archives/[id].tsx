...
export default function EventViewer(): React.JSX.Element {
    ...
    useEffect(() => {
        ...
        void new Archives().getById<Archive>(ARCHIVE).then((resp) => {
            ...
            setDataBlocks([
                {
                    label: "Temps passé" + (parseInt(durationEvent) !== 1 ? "s" : ""),
                    value: durationEvent,
                    icon: "ClockIcon",
                },
                {
                    label: "Crédit obtenu" + (parseInt(resp.evenement?.credits?.toString() ?? "0") > 1 ? "s" : ""),
                    value: (resp.evenement?.credits ?? 0),
        ...
        <View style={styles.inScrollContainer}>
                ...
                <View style={styles.subHeaderContainer}>
                    ...
                    <View style={styles.rightTextContainer}>
                        <Text style={styles.dateEventLabel}>
                            {archiveEvent?.start_date?.toLocaleDateString("fr-FR", {
                                dateStyle: "medium",
                            })}
                        </Text>
                    </View>
            ...
            <View
                style={[
                    ...
                    archivePosts.length === 0 ? {height: "auto"} : {},
                ]}
            >
                {archivePosts.length > 0 ? (
                    <>
                        <ImageGrid
                            images={archivePosts.map((image) => image.uri)}
                            onPress={onPress}
                        />
                        <ImageView
