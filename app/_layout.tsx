...
export default function RootLayout() {
    ...
    const [schoolColorsDynamic, setSchoolColorsDynamic] = useState(generateColors(defaultPrimary, defaultSecondary));
    ...
    useEffect(() => {
        if (isAuth === true) {
            void registerForPushNotificationsAsync()
                .then((token?: { type: "ios" | "android" | "web" | "windows" | "macos"; data: string }) => {
        ...
        void getData().then((storedData) => {
            ...
            if (storedData !== null) {
                if (storedData.permanentToken && storedData._id) {
                    void autoAuth(storedData.permanentToken, storedData._id).then((resp) => {
    ...
    const [theme, setTheme] = useState<boolean>(Appearance.getColorScheme() === "dark");
    ...
    return (
        <Provider store={store}>
            <SafeAreaProvider>
                ...
                {isAuth === null || !isStorageLoaded || (!isLoadingComplete && !fontsLoaded) ? (
                    <SplashScreenComponent onComplete={setSplashComplete}/>
                ) : splashComplete ? (
                    <>
