...
export const DraggableGrid = function <DataType extends IBaseItemType>(
  ...
  const [items] = useState<Array<IItem<DataType>>>([]);
  ...
  const assessGridSize = (event: IOnLayoutEvent): void => {
    if (!hadInitBlockSize) {
  ...
  function initBlockPositions(): void {
    items.forEach((_, index) => {
      blockPositions[index] = getBlockPositionByOrder(index);
    });
  }
  function getBlockPositionByOrder(order: number): IPositionOffset {
  ...
  function onHandRelease(): false | undefined {
    const activeItem = getActiveItem();
    if (!activeItem) return false;
  ...
  function resetBlockPositionByOrder(activeItemOrder: number, insertedPositionOrder: number): void {
  ...
  function moveBlockToBlockOrderPosition(itemKey: string | number): void {
  ...
  function getKeyByOrder(order: number): string {
    return findKey(orderMap, (item: IOrderMapItem) => item.order === order) as string;
  }

  function getSortData(): DataType[] {
  ...
  function getDistance(startOffset: IPositionOffset, endOffset: IPositionOffset): number {
  ...
  function setActiveBlock(itemIndex: number, item: DataType): void {
    if (item.disabledDrag) return;

    setPanResponderCapture(true);
    setActiveItemIndex(itemIndex);
  }
  function startDragStartAnimation(): void {
  ...
  function getActiveItem(): false | IItem<DataType> {
  ...
  function addItem(item: DataType, index: number): void {
