...
import { resourceMappingNoose } from "./ressourceMappingNoose";
import { type resourceMappingSkin } from "./ressourceMappingSkin";

interface NooseProps {
  selectedItem10: keyof typeof resourceMappingNoose;
  selectedItem12: keyof typeof resourceMappingSkin;
  setIsLoaded?: () => void;
}

export default function Noose(props: NooseProps) {
  const { selectedItem10, selectedItem12, setIsLoaded } = props;
  ...
  const {
    ...
    texture2,
    texture3,
  } = resourceMappingNoose[selectedItem10] || {};
  ...
  useEffect(() => {
    const loadObjectAndTexture = async () => {
      const [objResult, textureResult] = await Promise.all([
        ...
        loadTextureAsync({
          asset: selectedItem10 === "texture1" ? texture1 : texture,
        }),
      ]);
  ...
  }, [selectedItem10, selectedItem12, obj]);
