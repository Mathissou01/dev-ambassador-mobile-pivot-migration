...
export default function TabBarItem({
    ...
    const label =
        options.tabBarLabel !== undefined
            ...
            : options.title !== undefined
                ? options.title
                : item.label != undefined ? item.label : route.name;
    ...
    const secondaryColor = themeContext?.colors?.primary ?? getIconColorsForSchool("ESCEN").primaryColor;
    const primaryColor = themeContext?.colors?.secondary ?? getIconColorsForSchool("ESCEN").secondaryColor;

    const color = useThemeColor({light: colors.textLight, dark: colors.textDark});

    return (
        <TouchableOpacity onPress={onPress} activeOpacity={1} style={styles.container}>
            <Animatable.View useNativeDriver={true} ref={viewRef} style={styles.container}>
                <View
                    lightColor={colors.white}
                    darkColor={colors.backgroundNavDark}
                    style={[styles.btn, {borderColor: themeContext?.isDark ? colors.backgroundNavDark : colors.white}]}
                >
                    <Animatable.View
                        ...
                        style={[
                            styles.circle,
                            {
                                backgroundColor: `${themeContext?.colors?.primary ?? "#000000"}4D`,
                            },
                        ]}
                    />
                    <LottieView
                ...
                <Animatable.Text useNativeDriver={true} ref={textRef} style={[styles.text, {color: color}]}>
                    {label}
                </Animatable.Text>
            </Animatable.View>
        </TouchableOpacity>
