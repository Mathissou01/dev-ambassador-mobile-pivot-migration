...
export const getImageTransform = (image: Dimensions | null, screen: Dimensions) => {
  ...
  if (image.width > image.height) {
    scale = Math.min(screen.width / image.height, screen.height / image.width);
  }
...
export const getImageStyles = (
  ...
  if (scale) {
    if (image.width > image.height) transform.push({ rotate: "90deg" });
...
export const getImageTranslate = (image: Dimensions, screen: Dimensions): Position => {
...
export const getDistanceBetweenTouches = (touches: NativeTouchEvent[]): number => {
  const [a, b] = touches;
